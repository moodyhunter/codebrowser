...
public:
    PreprocessorCallback(Annotator &fm, clang::Preprocessor &PP, bool recoverIncludePath)
        : annotator(fm)
        , PP(PP)
        , recoverIncludePath(recoverIncludePath)
    {
    }
    ...
    virtual void Elif(clang::SourceLocation Loc, clang::SourceRange ConditionRange,
...
private:
    std::map<clang::SourceLocation, clang::SourceLocation> ElifMapping; // Map an elif location to
                                                                        // the real if;
